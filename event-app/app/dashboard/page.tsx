    ...
    <div className="min-h-screen bg-gray-100">
      <header className="bg-white shadow">
        <div className="max-w-6xl mx-auto px-4 py-4 flex justify-between items-center">
          <h1 className="text-xl font-bold text-black">Mes Événements</h1>
          <div className="space-x-4">
            <button onClick={() => setShowCreate(true)} className="bg-blue-600 text-white px-4 py-2 rounded hover:bg-blue-700">
              + Créer
            </button>
            <button onClick={logout} className="text-black hover:text-gray-800">
              Déconnexion
            </button>
          </div>
        </div>
      ...
      <main className="max-w-6xl mx-auto px-4 py-8">
        {showCreate && (
          <div className="bg-white p-6 rounded-lg shadow mb-6">
            <h2 className="text-lg font-semibold mb-4 text-black">Nouvel événement</h2>
            <form onSubmit={handleCreate} className="space-y-4">
              ...
              <div className="flex gap-2">
                ...
                <button type="button" onClick={() => setShowCreate(false)} className="px-4 py-2 border rounded text-black">
                  Annuler
                </button>
              </div>
            </form>
        ...
        <div className="grid gap-4 md:grid-cols-2 lg:grid-cols-3">
          {eventList.map((event) => (
            ...
            >
              ...
              {event.location && <p className="text-black text-sm">📍 {event.location}</p>}
        ...
        {eventList.length === 0 && (
          <p className="text-center text-black mt-8">Aucun événement. Créez-en un !</p>
        )}
      </main>
    </div>
  );
